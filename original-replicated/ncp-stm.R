...
library(foreign)
rawdata <- read.csv("original-replicated/ncp-stm-data.csv", header=T, sep=";", fileEncoding="latin1")
...
save(list = ls(all = TRUE), file = "results/preparedData-ncp-before-stm.RData")
...
png(filename = "results/SemanticCoherence_original.png", width = 10, height = 4, units = "in", res = 300)
...
save(list = ls(all = TRUE), file = "results/preparedModels-ncp-stm-original.RData")
...
png(filename = "reuslts/Figure1_original.png", width = 10, height = 4, units = "in", res = 300)
...
png(filename = "results/Figure2_original.png", width = 10, height = 4, units = "in", res = 300)
...
# and save it for XLS
write.table(meta, "results/dataset-thetas.txt", sep="\t") 
